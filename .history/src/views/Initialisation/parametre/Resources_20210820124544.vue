...
<section class="tabs-section">
							...
							<li class="nav-item">
								<a class="nav-link active show" href="#tabs-2-tab-1" role="tab" data-toggle="tab" aria-selected="true">
									<span class="nav-link-in">
										Resource Humaine
										<span class="label label-pill label-success">{{getterUsers.length}}</span>
									</span>
								</a>
							</li>
							<li class="nav-item" v-for="item in listeResourceDiffRH" :key="item.id">
								<a class="nav-link" :href="'#'+item.id" role="tab" data-toggle="tab" aria-selected="false">
									<span class="nav-link-in">
										{{item.libelle}}
          ...
          <div class="card-user-photo">
							<img :src="imageUser(user.photo)" style="width:30px">
						</div>
          </td>
          <td>{{user.code}}</td>
					<td>{{user.libelle}}</td>
          	...
          	<td>{{fonctionLocale(user.fonction_locale_id)}}</td>
      ...
      >
        mdi-delete
					...
					<div role="tabpanel" class="tab-pane fade"  v-for="item in listeResourceDiffRH" :id="item.id" :key="'TY'+item.id">
...
<table id="table-sm" class="table table-bordered table-hover table-sm">
				...
				<tbody>
				<tr v-for="resource in listeResource(item.id)" :key="resource.id">
					<td>{{resource.code}}</td>
					<td>{{resource.libelle}}</td>
          ...
          <v-card>
            ...
            <v-card-text>
                ...
                <div class="form-group">
                   <label class="form-label semibold" for="type">Type de resource</label>
              <select id="type" :class="{ 'hasErroInpu': $v.formData.type_resource_id.$error }" class="form-control" v-model="formData.type_resource_id">
							 	<option value="">Selectionner le type de resource</option>
								<option :value="projetPartie.id" v-for="projetPartie in getterTypeResource" :key="'DFR'+projetPartie.id">{{projetPartie.libelle}}</option>
							</select>
                 <div style="color: red;" class="" v-if="$v.formData.type_resource_id.$error">
              ...
              <div style="color: red;" class="" v-if="$v.formData.code.$error">
                   ...
                   <div class="form-group">
                         <label class="form-label semibold" for="libelle">Nom et prenom ou désignation</label>
...
<div class="row" v-if="verifieTypeResource(formData.type_resource_id)">
   <div class="col-md-6">
     <div class="form-group" >
                  <label class="form-label semibold" for="email">Email</label>
					    		<input type="text"  v-model="email" name="email" class="form-control" id="email" placeholder="email">
                </div>
   ...
   <div class="col-md-6">
       ...
       <div class="form-group">
                   <label class="form-label semibold" for="telephone">Télephone</label>
						        <input type="text"  v-model="telephone" name="telephone" class="form-control" id="telephone" placeholder="Telephone">
                </div>
   </div>
   <div class="col-md-12">
<div class="form-group">
             <label class="form-label semibold" for="fonction">Fonction locale</label>
              <select id="fonction"  class="form-control" v-model="formData.fonction_locale_id">
							 	<option value="">Selectionner la fonction locale</option>
								<option :value="projetPartie.id" v-for="projetPartie in getterFonctionLocale" :key="'D'+projetPartie.id">{{projetPartie.libelle}}</option>
							</select>
            </div>
   </div>
</div>
             ...
             <div class="form-group">
                            <label class="form-label semibold" for="chef_projet">Coût  </label>
                            <money v-model="formData.cout" name="budget" class="form-control" id="budget" placeholder="Côut"  ></money>
        ...
        <template v-slot:default="dialogEdite">
          <v-card>
            ...
            <v-card-text>
                ...
                <div class="form-group">
                   <label class="form-label semibold" for="type">Type de resource</label>
              <select id="type"  class="form-control" v-model="edite.type_resource_id">
							 	<option value="">Selectionner le type de resource</option>
								<option :value="projetPartie.id" v-for="projetPartie in getterTypeResource" :key="'DF2R'+projetPartie.id">{{projetPartie.libelle}}</option>
							</select>
           ...
           <div class="row" v-if="verifieTypeResource(edite.type_resource_id)">
   <div class="col-md-6">
     <div class="form-group" >
                  <label class="form-label semibold" for="email">Email</label>
...
<div class="form-group">
             <label class="form-label semibold" for="fonction">Fonction locale</label>
              <select id="fonction"  class="form-control" v-model="edite.fonction_locale_id">
							 	<option value="">Selectionner la fonction locale</option>
								<option :value="projetPartie.id" v-for="projetPartie in getterFonctionLocale" :key="'D'+projetPartie.id">{{projetPartie.libelle}}</option>
							</select>
            </div>
             ...
             <div class="form-group">
                            <label class="form-label semibold" for="chef_projet">Coût  </label>
                            <money v-model="edite.cout" name="budget" class="form-control" id="budget" placeholder="Côut"  ></money>
     ...
     dialogEdite: false,
      ...
      defaultItem: {
       code: '',
        ...
        computed:{
            ...mapGetters('Application', ['getterTypeResource', 
            'getterLoadingResource',"getterResource",
            "getterErrorResource","getterFonctionLocale","getterUsers","getterLoadingUsers","getterErrorUsers"]),
              formTitle () {
           ...
           verifieTypeResource(){
             return type_resource_id=>{
                if(type_resource_id=="") return false
                 let objet=this.getterTypeResource.find(item=>item.id==type_resource_id)
           ...
           listeResource(){
             return id=>{
               if(id==null) return []
               let objet=this.getterResource.filter(item=>item.type_resource_id==id)
        ...
        methods:{
             ...mapActions('Application', ['ajouterResource',"supprimerResource","modificationResource","ajouterUser","supprimerUser","modificationUser"]),
            gotoModale(){
               this.dialog=true 
            }
            ,
