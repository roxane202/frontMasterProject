...
<v-app>
 ...
 <v-data-table
  ...
  <template v-slot:[`item.budget`]="{ item }">
      ...
      <div>
        {{formatageSomme(item.budget)}}
      </div>
    </template>
    <template v-slot:[`item.devise_id`]="{ item }">
    
      <div>
        {{deviseProjet(item.devise_id)}}
      </div>
    </template>
          ...
          <div class="col-md-6">
           ...
           <div class="col-md-4">
             <div class="form-group">
              <select id="partie" class="form-control" v-model="projetPP.partie_prenante_id">
							 	<option value="">Selectionner Partie prenante</option>
								<option :value="projetPartie.id" v-for="projetPartie in partiePrenanteNoExisteToProjet(item.id)" :key="'DFR'+projetPartie.id">{{projetPartie.designation}}</option>
							</select>
             </div>
...
>Ajouter</v-btn></div>
            ...
            <table id="table-edit" class="table table-hover">
                <tr>
                    ...
                    <td><v-icon
                        small
                        @click="deleteItemProjetPartie(partie)"
                      >
                mdi-delete
             </v-icon>
      </td> 
                ...
                <div class="row">
                    ...
                    <div class="form-group">
                            <label class="form-label semibold" for="date_debut">Date début</label>
                                <input type="date" :class="{ 'hasErroInpu': $v.formData.date_debut.$error }" v-model="formData.date_debut" name="date_debut" class="form-control" id="date_debut" placeholder="Date début">
                                <div style="color: red;" class="" v-if="$v.formData.date_debut.$error">
                                    Veillez entrer la date début
                                </div>
                    </div>
                    ...
                    <div class="form-group">
                            <label class="form-label semibold" for="chef_projet">Chef de projet</label>
                                <input type="text" :class="{ 'hasErroInpu': $v.formData.chef_projet.$error }" v-model="formData.chef_projet" name="chef_projet" class="form-control" id="date_debut" placeholder="Chef de projet">
                                <div style="color: red;" class="" v-if="$v.formData.chef_projet.$error">
                                    Veillez entrer chef de projet
                                </div>
                        </div>
                      ...
                      <div class="form-group">
                              ...
                              <select id="devise" class="form-control" v-model="formData.devise_id">
								<option></option>
								<option :value="item.id" v-for="item in getterDevise" :key="item.id">{{item.libelle}}</option>
							  </select>
                        </div>
                ...
                <div style="color: red;" class="" v-if="$v.formData.designation.$error">
                    ...
                    <div class="form-group">
                            <label class="form-label semibold" for="date_fin">Date fin</label>
                                <input type="date"  :min="formData.date_debut" :class="{ 'hasErroInpu': $v.formData.date_fin.$error }" v-model="formData.date_fin" name="date_fin" class="form-control" id="date_fin" placeholder="Date fin">
                                <div style="color: red;" class="" v-if="$v.formData.date_fin.$error">
                                    Veillez entrer la date fin
                                </div>
                        </div>
                         <div class="form-group">
                            <label class="form-label semibold" for="chef_projet">Commanditaire</label>
                                <input type="text" :class="{ 'hasErroInpu': $v.formData.commanditaire.$error }" v-model="formData.commanditaire" name="commanditaire" class="form-control" id="commanditaire" placeholder="commanditaire">
                                <div style="color: red;" class="" v-if="$v.formData.commanditaire.$error">
                                    Veillez entrer Commanditaire
                                </div>
                        </div>

                        <div class="form-group">
                            <label class="form-label semibold" for="chef_projet">Budget projet  {{deviseProjet(formData.devise_id)}}</label>
                              
                                <money v-model="formData.budget" name="budget" class="form-control" id="budget" placeholder="budget" :class="{ 'hasErroInpu': $v.formData.budget.$error }" ></money>
                                <div style="color: red;" class="" v-if="$v.formData.budget.$error">
                                    Veillez entrer budget
                                </div>
                        </div>
...
<!--DIALOG LOADING--->
 ...
 <v-dialog
              v-model="dialogDeleteProjetPartie"
              max-width="290"
            >
              <v-card >
                <v-card-title class="headline">
                ...
                <v-card-actions>
                  ...
                  <v-btn
                    color="green"
                    text
                    @click="dialogDeleteProjetPartie = false"
                  >
                    Annuler
                  </v-btn>
                ...
                <div class="row">
                    ...
                    <div class="form-group">
                            <label class="form-label semibold" for="date_debut">Date debut</label>
                                <input type="date"  v-model="edite.date_debut" name="date_debut" class="form-control" id="date_debut" placeholder="Date debut">
                    ...
                    <div class="form-group">
                            <label class="form-label semibold" for="chef_projet">Chef de projet</label>
                                <input type="text" v-model="edite.chef_projet" name="chef_projet" class="form-control" id="date_debut" placeholder="Chef de projet">
                     ...
                     <div class="col-md-6">
                           <div class="form-group">
                <label class="form-label semibold" for="code">Désignation</label>
                                <input type="text"  v-model="edite.designation" name="designation" class="form-control" id="designation" placeholder="designation">
                
                    </div>

                    <div class="form-group">
                            <label class="form-label semibold" for="date_fin">Date fin</label>
                                <input type="date"  :min="edite.date_debut"  v-model="edite.date_fin" name="date_fin" class="form-control" id="date_fin" placeholder="Date fin">
                                
                        </div>
                         <div class="form-group">
                            <label class="form-label semibold" for="chef_projet">Commanditaire</label>
                                <input type="text"  v-model="edite.commanditaire" name="commanditaire" class="form-control" id="commanditaire" placeholder="commanditaire">
                                
                        </div>

                        <div class="form-group">
                            <label class="form-label semibold" for="chef_projet">Budget projet  {{deviseProjet(edite.devise_id)}}</label>
                              
                                <money v-model="edite.budget" name="budget" class="form-control" id="budget" placeholder="budget"  ></money>
    ...
    export default {
     ...
     dialogEdite: false,
      ...
      defaultItem: {
        ...
        computed:{
            ...mapGetters('Application', ["getterPartiePrenanteProjet",'getterProjet',
             'getterLoadingProjet',"getterErrorProjet",
             "getterDevise","getterLoadingProjetPartiePrenantes","getterProjetPartiePrenantes","getterErrorProjetPartiePrenantes"]),
             deviseProjet(){
                 return id=>{
             ...
             listePartiePrenanteProjet(){
               return id=>{
                 if(id==null) return []
                 let objetProjetPartiePrenante=this.getterProjetPartiePrenantes.filter(item=>item.projet_id==id)
                 
                let vm=this
                 let collection=[]
                     objetProjetPartiePrenante.forEach(function(value){
                       let objet=vm.getterPartiePrenanteProjet.find(ob=>ob.id==value.partie_prenante_id)
                       if(objet!=undefined){
                         let ob={
                           ...objet,
                 ...
                 this.getterPartiePrenanteProjet.forEach(function(value){
                   console.log(value)
                    let objet=vm.getterProjetPartiePrenantes.find(item=>{
                      if(item.partie_prenante_id==value.id && item.projet_id==projet){
                            return item
                      }
        ...
        methods:{
             ...mapActions('Application', ['ajouterProjet',"supprimerProjet","modificationProjet",
             "ajouterProjetPartiePrenante","supprimerProjetPartiePrenante",
